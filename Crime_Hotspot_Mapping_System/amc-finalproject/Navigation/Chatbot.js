...
const handleSend = async () => {
  ...
  const interval = setInterval(() => {
    ...
    setMessages(prev => {
      const updated = [...prev];
      updated[updated.length - 1] = { text: `Loading${dots}`, sender: 'bot', isLoading: true };
      return updated;
    });
  }, 500);
  ...
  contents: [
    {
      parts: [
        {
          text: `The user is asking a question. If it is related to crime, answer it concisely. 
                  Otherwise, respond with: "This topic is not related to the crime."\n\nQuestion: ${inputText}`
        }
      ]
    }
  ]
    ...
    setMessages(prev => {
      const updated = [...prev];
      updated[updated.length - 1] = { text: responseText, sender: 'bot' };
      return updated;
    });
    ...
    setMessages(prev => {
      const updated = [...prev];
      updated[updated.length - 1] = { text: 'Sorry, ako ang MALI', sender: 'bot' };
      return updated;
    });
  } finally {
    setLoading(false);
